...
- Run `nix run .#setup-entire` after `nix develop` to let Entire install hooks
  and enable the configured agents.
- `nix run .#setup-entire` creates repo config like `.entire/` and
  agent-specific config directories such as `.opencode/`; commit those in the
  generated project.
